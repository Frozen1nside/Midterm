class GRUModel {
    ...
    async trainModel(features, labels, epochs = 100, validationSplit = 0.2) {
        try {
            // Convert to tensors and reshape for GRU (samples, timesteps, features)
    ...
    async predict(features) {
        ...
        return result[0] * 10; // Denormalize back to 0-10 scale
    }
    ...
    evaluateModel(testFeatures, testLabels) {
        ...
        return {
            loss: loss,
            rmse: Math.sqrt(loss) * 10 // Convert to original scale
        };
    }
